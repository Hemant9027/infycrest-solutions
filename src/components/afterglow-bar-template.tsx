  ...
  return (
    <p
      className={cx(
        ...
        tone === "dark" ? "text-[var(--brand-accent)]" : "text-[#a05c35]",
        className,
      )}
    >
      <span
        className={cx(
          "h-px w-10",
          tone === "dark" ? "bg-[var(--brand-accent)]/60" : "bg-[#a05c35]/50",
        )}
      />
      <span>{index}</span>
...
export function SpinBadge({
  ...
  return (
    <div className={cx("relative grid place-items-center", className)}>
      ...
      <Asterisk className="absolute size-5 text-[var(--brand-accent)]" />
    </div>
  );
}
...
export function Stat({ value, label }: { value: string; label: string }) {
  ...
  return (
    <div ref={ref}>
      ...
      <p className="text-[10px] uppercase tracking-[0.25em] text-[var(--brand-accent)]">
        {label}
      </p>
    </div>
  );
      ...
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 lg:px-10">
          <a href="#top" className="group flex items-baseline gap-2">
            <Asterisk className="size-4 translate-y-[1px] text-[var(--brand-accent)] transition-transform duration-700 group-hover:rotate-180" />
          ...
          <nav className="hidden items-center gap-10 md:flex">
            {NAV_LINKS.map((link) => (
              ...
              >
                <sup className="mr-1.5 text-[8px] text-[var(--brand-accent)]/80">
                  {link.index}
                </sup>
                {link.label}
                <span className="absolute -bottom-1.5 left-0 h-px w-0 bg-[var(--brand-accent)] transition-all duration-500 ease-out group-hover:w-full" />
              </a>
            ))}
          </nav>
          ...
          <a
            href="#reserve"
            className="hidden items-center gap-2 rounded-full border border-[var(--brand-accent)] px-6 py-2.5 text-[10px] uppercase tracking-[0.25em] text-[var(--brand-accent)] transition-all duration-300 hover:bg-[var(--brand-accent)] hover:text-[#181315] md:inline-flex"
          >
          ...
          <button
            ...
            onClick={() => setOpen((v) => !v)}
            className="grid size-10 place-items-center rounded-full border border-white/20 text-white transition-colors hover:border-[var(--brand-accent)] hover:text-[var(--brand-accent)] md:hidden"
          >
        ...
        {/* Scroll progress */}
        <span
          className="absolute bottom-0 left-0 block h-px bg-[var(--brand-accent)] transition-[width] duration-150 ease-out"
            ...
            <div key={link.href} className="overflow-hidden">
              <a
                ...
                className={cx(
                  ...
                  link.href === "#reserve" && "text-[var(--brand-accent)]",
                )}
              >
                <span className="text-xs tracking-[0.3em] text-[var(--brand-accent)]/70">
                  {link.index}
                </span>
                {link.label}
              </a>
...
export function Hero({ customer }: { customer: NewCustomer }) {
  ...
  return (
    <section
      id="top"
      className="relative flex h-[100svh] min-h-[720px] items-end overflow-hidden"
    >
      {slides.map((img, idx) => (
        <div
          key={`${img}-${idx}`}
          className={cx(
            "absolute inset-0 transition-opacity ease-in-out [transition-duration:1800ms]",
      ...
      <div className="absolute inset-0 z-10 bg-gradient-to-t from-[#181315] via-[#181315]/40 to-[#181315]/10" />
      <div className="absolute inset-0 z-10 bg-gradient-to-r from-[#181315]/70 via-transparent to-transparent" />
      <div
      ...
      <div className="relative z-20 mx-auto w-full max-w-7xl px-6 pb-28 lg:px-10 lg:pb-32">
        <div className="block overflow-hidden">
          <p
            ...
            className={cx(
              "mb-7 flex items-center gap-3 text-[10px] uppercase tracking-[0.4em] text-[var(--brand-accent)] transition-transform duration-[1100ms] ease-[cubic-bezier(0.19,1,0.22,1)]",
              mounted ? "translate-y-0" : "translate-y-[130%]",
            )}
          >
            <span className="inline-block size-1.5 animate-pulse rounded-full bg-[var(--brand-accent)]" />
            {text(
        ...
        <h1 className="max-w-5xl font-serif text-[clamp(3.4rem,9.5vw,8.5rem)] font-light leading-[0.92] text-[#f7eee4]">
          <span className="block overflow-hidden pb-1">
            <span
              ...
              className={cx(
                "block transition-transform duration-[1300ms] ease-[cubic-bezier(0.19,1,0.22,1)]",
          ...
          <span className="block overflow-hidden pb-2">
            ...
            >
              <em className="italic text-[var(--brand-accent)]">{tail}</em>
            </span>
          </span>
        </h1>
        ...
        >
          <a
            href="#reserve"
            className="group inline-flex items-center gap-3 rounded-full bg-[var(--brand-accent)] px-8 py-4 text-[10px] font-bold uppercase tracking-[0.25em] text-[#181315] transition-all duration-300 hover:bg-white"
          >
          ...
          <a
            href="#menu"
            className="group inline-flex items-center gap-2 text-[10px] uppercase tracking-[0.3em] text-white/60 transition-colors hover:text-[var(--brand-accent)]"
          >
            Explore the menu
            ...
            >
              ...
              <span className="relative h-px w-16 overflow-hidden bg-white/15">
                {idx === current && (
                  <span
                    key={current}
                    className="block h-full animate-fillbar bg-[var(--brand-accent)]"
                  />
                )}
              </span>
            </button>
      ...
      <div className="absolute bottom-0 left-6 z-20 hidden flex-col items-center gap-3 lg:left-10 lg:flex">
        ...
        <span className="relative h-16 w-px bg-white/15">
          <span className="absolute left-1/2 top-0 size-[3px] -translate-x-1/2 animate-scroll-dot rounded-full bg-[var(--brand-accent)]" />
        </span>
      </div>
    </section>
  );
...
export function Ticker({ items }: { items: string[] }) {
    ...
    <div aria-hidden={hidden} className="flex shrink-0 items-center">
      {list.map((item, i) => (
        <span key={`${item}-${i}`} className="flex items-center">
          ...
          <span className="mx-8 inline-block size-1.5 rotate-45 bg-[var(--brand-accent)]" />
        </span>
      ))}
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-y-0 left-0 w-24 bg-gradient-to-r from-[#141012] to-transparent" />
      <div className="pointer-events-none absolute inset-y-0 right-0 w-24 bg-gradient-to-l from-[#141012] to-transparent" />
    </section>
  );
}
    ...
    >
      <div className="grid gap-16 lg:grid-cols-12 lg:gap-12">
          ...
          <div className="lg:sticky lg:top-32">
            ...
            <Reveal delay={120}>
              <h2 className="mt-8 font-serif text-5xl font-light leading-[1.04] text-[#f7eee4] sm:text-6xl">
                {titleHead}{" "}
                <em className="italic text-[var(--brand-accent)]">
                  {titleTail}
                </em>
              </h2>
            </Reveal>
            <Reveal delay={220}>
              <p className="mt-9 max-w-md text-[15px] font-light leading-[1.9] text-[#f7eee4]/60 first-letter:float-left first-letter:mr-3 first-letter:font-serif first-letter:text-[3.4rem] first-letter:font-light first-letter:leading-[0.85] first-letter:text-[var(--brand-accent)]">
                {text(
                  customer.about?.body,
        ...
        <div className="relative lg:col-span-7">
          <Reveal delay={150} className="relative">
            <div className="relative w-[84%]">
              ...
              <div className="relative aspect-[4/5] overflow-hidden bg-[#201a1c]">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#181315]/45 via-transparent to-transparent" />
              </div>
              <p className="mt-4 flex items-center gap-3 text-[10px] uppercase tracking-[0.3em] text-white/40">
                <span className="h-px w-8 bg-[var(--brand-accent)]/60" />— 01 ·
                The back bar, mid-set
              </p>
            </div>
          </Reveal>
          ...
          >
            <div className="relative aspect-[4/3] overflow-hidden border-[10px] border-[#181315] bg-[#201a1c] shadow-[0_30px_80px_rgba(0,0,0,0.5)]">
              <Image
                src={detailImage}
            ...
            <p className="mt-4 flex items-center justify-end gap-3 text-[10px] uppercase tracking-[0.3em] text-white/40">
              ...
              <span className="h-px w-8 bg-[var(--brand-accent)]/60" />
            </p>
          </Reveal>
              ...
              {items.map((item, index) => (
                <li key={`${item.title}-${index}`}>
                  <a
                    ...
                    className="group grid grid-cols-[auto_1fr_auto] items-baseline gap-5 border-b border-[#181315]/15 py-7 transition-colors duration-500 hover:bg-[#181315]/[0.04] sm:gap-8 sm:py-8"
                  >
          ...
          <div>
            ...
            <Reveal delay={120}>
              <h2 className="mt-8 font-serif text-5xl font-light leading-[1.02] text-[#f7eee4] sm:text-6xl">
                Step inside{" "}
                <em className="italic text-[var(--brand-accent)]">the hour.</em>
              </h2>
            </Reveal>
          </div>
          <Reveal delay={220}>
          ...
          {frames.map((frame, idx) => (
            ...
            >
              ...
              >
                ...
                />
                <span className="absolute inset-0 bg-gradient-to-t from-[#181315]/80 via-transparent to-transparent opacity-0 transition-opacity duration-700 group-hover:opacity-100" />
                <span className="absolute inset-x-0 bottom-0 flex translate-y-3 items-center justify-between p-4 opacity-0 transition-all duration-500 group-hover:translate-y-0 group-hover:opacity-100">
                  <span className="text-left">
                    <span className="block text-[9px] uppercase tracking-[0.3em] text-[var(--brand-accent)]">
      ...
      {openIdx !== null && (
        ...
        >
          <button
            ...
            aria-label="Close"
            onClick={close}
            className="absolute right-6 top-6 grid size-11 place-items-center rounded-full border border-white/15 text-white/70 transition-colors hover:border-[var(--brand-accent)] hover:text-[var(--brand-accent)]"
          >
            <X className="size-4" />
          </button>
          <button
            ...
            className="absolute left-4 top-1/2 grid size-11 -translate-y-1/2 place-items-center rounded-full border border-white/15 text-white/70 transition-colors hover:border-[var(--brand-accent)] hover:text-[var(--brand-accent)] sm:left-8"
          >
          ...
          <button
            ...
            className="absolute right-4 top-1/2 grid size-11 -translate-y-1/2 place-items-center rounded-full border border-white/15 text-white/70 transition-colors hover:border-[var(--brand-accent)] hover:text-[var(--brand-accent)] sm:right-8"
          >
    ...
    >
      <div>
        ...
        <Reveal delay={200}>
          <p className="mt-7 flex max-w-sm items-start gap-3 text-sm font-light leading-relaxed text-white/50">
            <Sparkles className="mt-0.5 size-4 shrink-0 text-[var(--brand-accent)]" />
          ...
          <div className="mt-12 space-y-7">
            {contactRows.map((row) => (
              <div key={row.label} className="group flex items-start gap-5">
                <span className="grid size-11 shrink-0 place-items-center rounded-full border border-white/10 text-[var(--brand-accent)] transition-colors duration-500 group-hover:border-[var(--brand-accent)]/60">
                ...
                <div>
                  ...
                  {row.href ? (
                    <a
                      href={row.href}
                      className="text-sm font-light text-white/80 transition-colors hover:text-[var(--brand-accent)]"
                    >
                      {row.value}
                    </a>
                  ) : (
      ...
      <Reveal delay={180}>
        <div className="relative border border-white/10 bg-[#201a1c] p-8 shadow-[0_40px_100px_rgba(0,0,0,0.35)] sm:p-12">
          <span className="absolute -top-px left-10 right-10 h-px bg-gradient-to-r from-transparent via-[var(--brand-accent)]/70 to-transparent" />
          <SpinBadge
          ...
          {status === "sent" && summary ? (
            <div className="flex flex-col items-center py-14 text-center">
              ...
              <button
                ...
                className="mt-9 text-[10px] uppercase tracking-[0.3em] text-[var(--brand-accent)] underline-offset-4 hover:underline"
              >
            ...
            <>
              ...
              >
                ...
                <button
                  ...
                  className={cx(
                    "group mt-2 inline-flex items-center justify-center gap-3 rounded-full bg-[var(--brand-accent)] px-6 py-4 text-[10px] font-bold uppercase tracking-[0.25em] text-[#181315] transition-all duration-300 sm:col-span-2",
                    status === "sending"
          ...
          <div className="grid gap-14 pb-20 md:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr_1fr]">
            <div>
              <a href="#top" className="flex items-baseline gap-2">
                <Asterisk className="size-4 translate-y-[1px] text-[var(--brand-accent)]" />
                <span className="font-serif text-xl uppercase tracking-[0.22em] text-[#f7eee4]">
                  {customer.businessName}
                </span>
              </a>
              ...
              <div className="mt-7 flex flex-wrap gap-x-6 gap-y-2">
                {socials.map((social) => (
                  <a
                    key={social}
                    href="#top"
                    className="group inline-flex items-center gap-1.5 text-[10px] uppercase tracking-[0.28em] text-white/50 transition-colors duration-300 hover:text-[var(--brand-accent)]"
                  >
                    {social}
            ...
            <div>
              <p className="text-[10px] uppercase tracking-[0.3em] text-[var(--brand-accent)]">
                Explore
              </p>
      ...
      <div className="border-t border-white/5">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-6 py-6 text-[10px] uppercase tracking-[0.28em] text-white/35 sm:flex-row lg:px-10">
          ...
          <a
            ...
            className="grid size-10 place-items-center rounded-full border border-white/10 text-white/50 transition-all duration-300 hover:border-[var(--brand-accent)] hover:bg-[var(--brand-accent)] hover:text-[#181315]"
          >
...
export function PremiumBarTemplate({
  ...
  return (
    <div
      style={style}
      className="bg-[#181315] font-sans text-[#f7eee4] selection:bg-[var(--brand-accent)] selection:text-[#181315]"
    >
